...
class RunningMeanStd:
    """Welford running mean/variance for return normalization (SB3-style).

    Used to standardize TD targets so the Huber loss cannot explode when
    Q-values drift upward (the divergence mode seen on the vocab=1 run). We
    scale BOTH prediction and target by the same running std, which only
    rescales the gradient magnitude -- argmax/greedy policy and the mixer's
    monotonicity are scale-invariant, so the learned policy is unchanged while
    runaway is bounded.
    """
...
# Module-level singleton (one training process == one run under Hydra).
...
def update_comm_qmix(batch, mac, target_mac, mixer, target_mixer, optimizer, all_params, cfg, device, hidden_dim, tau, msg_decoder=None):
    ...
    # STABILITY: Huber (smooth-L1) instead of MSE caps the per-sample gradient
    # for large TD errors -> blunts the Q-overestimation runaway that diverged
    # the vocab=1 run. Optional return normalization (cfg.agent.normalize_returns)
    # standardizes both sides by a running std so the loss can't explode as Q
    # drifts; the std clamp keeps small-target episodes from being amplified.
    if cfg.agent.get("normalize_returns", False):
        _RET_RMS.update(targets)
        scale = max(_RET_RMS.std, 1.0)
